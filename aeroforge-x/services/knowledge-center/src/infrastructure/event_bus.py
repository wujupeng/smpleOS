...
class EventBus:
        ...
        streams = {
            "KNOWLEDGE": {
                "subjects": [
                    ...
                    "knowledge.link.updated",
                    "knowledge.link.deleted",
                    "knowledge.impact.propagated",
                    "knowledge.inference.completed",
                    "knowledge.snapshot.created",
                    "knowledge.snapshot.restored",
                    "knowledge.anomaly.detected",
                    "knowledge.quality.assessed",
                ],
            },
        }
        for name, config in streams.items():
            try:
                await self._js.add_stream(
                    name=name, subjects=config["subjects"]
                )
            except Exception:
                pass
    ...
    async def subscribe(
        self, subject: str, handler: Callable[[dict], Any], durable: str = None
    ):
        if not self._js:
            raise RuntimeError("JetStream not initialized")
