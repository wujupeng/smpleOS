...
class OpenFOAMAdapter(AeroDataImportAdapter):
    ...
    def parse(self, file_path: str) -> RawAeroData:
        ...
        metadata: dict[str, Any] = {"source": "OpenFOAM", "file_path": file_path}
        missing_dimensions: list[str] = []

        config_match = re.search(r"Case:\s*(.+)", content)
        if config_match:
            metadata["config"] = config_match.group(1).strip()

        alpha_pattern = re.compile(r"alpha\s*=\s*([-\d.]+)", re.IGNORECASE)
        mach_pattern = re.compile(r"(?:Mach|Ma)\s*=\s*([\d.]+)", re.IGNORECASE)
        beta_pattern = re.compile(r"(?:beta|sideslip)\s*=\s*([-\d.]+)", re.IGNORECASE)
        re_pattern = re.compile(r"(?:Re|Reynolds)\s*=\s*([\d.eE+]+)", re.IGNORECASE)
        ...
        for m in re_pattern.finditer(content):
            val = float(m.group(1))
            if val not in reynolds_values:
                reynolds_values.append(val)
        ...
        force_pattern = re.compile(
            r"(?:Total|Forces).*?Fx\s*=\s*([-\d.eE+]+).*?Fy\s*=\s*([-\d.eE+]+).*?Fz\s*=\s*([-\d.eE+]+)",
            re.IGNORECASE | re.DOTALL,
        )
        moment_pattern = re.compile(
            r"(?:Total|Moments).*?Mx\s*=\s*([-\d.eE+]+).*?My\s*=\s*([-\d.eE+]+).*?Mz\s*=\s*([-\d.eE+]+)",
            re.IGNORECASE | re.DOTALL,
        )

        coeff_direct_patterns = {
            "CL": re.compile(r"(?:CL|C_L|lift_coeff)\s*=\s*([-\d.eE+]+)", re.IGNORECASE),
            "CD": re.compile(r"(?:CD|C_D|drag_coeff)\s*=\s*([-\d.eE+]+)", re.IGNORECASE),
            "CM": re.compile(r"(?:CM|C_M|pitch_coeff)\s*=\s*([-\d.eE+]+)", re.IGNORECASE),
            "CY": re.compile(r"(?:CY|C_Y|side_coeff)\s*=\s*([-\d.eE+]+)", re.IGNORECASE),
            "Cl": re.compile(r"(?:Cl_roll|C_l_roll|roll_coeff)\s*=\s*([-\d.eE+]+)", re.IGNORECASE),
            "Cn": re.compile(r"(?:Cn_yaw|C_n_yaw|yaw_coeff)\s*=\s*([-\d.eE+]+)", re.IGNORECASE),
        }

        blocks = re.split(r"(?=alpha\s*=)", content)
        for block in blocks:
            if "alpha" not in block.lower() and "case" not in block.lower():
                continue

            alpha_m = alpha_pattern.search(block)
            mach_m = mach_pattern.search(block)
            beta_m = beta_pattern.search(block)
            re_m = re_pattern.search(block)

            alpha_val = float(alpha_m.group(1)) if alpha_m else alpha_values[0]
            ...
            for coeff_name, pattern in coeff_direct_patterns.items():
                match = pattern.search(block)
                if match:
                    coefficients[coeff_name][i_alpha, i_beta, i_mach, i_reynolds] = float(match.group(1))

            force_m = force_pattern.search(block)
            if force_m and "CL" not in coefficients or np.all(coefficients["CL"] == 0):
                metadata["has_raw_forces"] = True

            moment_m = moment_pattern.search(block)
            if moment_m and "CM" not in coefficients or np.all(coefficients["CM"] == 0):
                metadata["has_raw_moments"] = True
    ...
    def convert_to_internal(self, raw: RawAeroData) -> FourDimLookupTable:
        ...
        table = FourDimLookupTable(
            database_id=f"ADB-OpenFOAM-{raw.metadata.get('config', 'default')}",
            database_name=f"OpenFOAM Import ({raw.metadata.get('file_path', 'unknown')})",
